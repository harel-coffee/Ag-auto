...
sum_no_years = len(ndvi_modis.year.unique())

for a_county in ndvi_modis.county.unique():
    df = ndvi_modis[ndvi_modis.county == a_county]
    if len(df.year.unique()) != sum_no_years:
...
sum_no_years = len(ndvi_avhrr.year.unique())
...
sum_no_years = len(ndvi_gimms.year.unique())
