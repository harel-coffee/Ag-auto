...
# # new version of above cell
# Here we have one column to indicate most important season for each county
...
for file_name in Min_files:
    ...
    county_seasons.fillna(0, inplace=True)
    county_seasons.head(2)

    county_seasons.rename(columns={"s1": "s1_" + satellite,
                                   "s2": "s2_" + satellite,
                                   "s3": "s3_" + satellite,
                                   "s4": "s4_" + satellite}, inplace=True)

    cs = [x + satellite for x in ["s1_", "s2_", "s3_", "s4_"]]
    for c in cs:
        county_seasons[c] = np.where(county_seasons[c].isin([1, 2, 3, 4]), 1, county_seasons[c])
        county_seasons[c] = county_seasons[c].astype(int)
    ...
    county_seasons = pd.melt(county_seasons, id_vars=['county_fips'])
    county_seasons = county_seasons[county_seasons.value==1]

    county_seasons.rename(columns={"variable": "most_imp_season_" + satellite}, inplace=True)
    county_seasons = county_seasons[["county_fips", "most_imp_season_" + satellite]].copy()
    county_seasons.head(2)
...
print (ndvi_all.shape)
ndvi_all.head(2)

ndvi_all_cell8 = ndvi_all.copy()
