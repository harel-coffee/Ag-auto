...
# %%
State_NDVIDOY_Herb = pd.read_csv(data_dir_base + "Supriya/" + "State_NDVIDOY_Herb_Feb82024.csv")

State_NDVIDOY_Herb.rename(columns=lambda x: x.lower().replace(' ', '_'), inplace=True)

cols_ = ['statefp', 'stusps', 'herbcount', 'herbmean', 'herbstdev', 'vi_doymean',
         'vi_doymedi', 'vi_doystde', 'vi_doymin', 'vi_doymax']
...
# %%
State_NDVIDOY_Herb.rename(columns={"statefp": "state_fips",
                                   "stusps" : "state",
...
State_NDVIDOY_Herb = rc.correct_state_int_fips_to_str(df=State_NDVIDOY_Herb, col_="state_fips")
State_NDVIDOY_Herb.head(2)
...
# %%
County_NDVIDOY_Herb = pd.read_csv(data_dir_base + "Supriya/" + "County_NDVIDOY_Herb_Feb82024.csv", \
                                           encoding='unicode_escape')
County_NDVIDOY_Herb.rename(columns=lambda x: x.lower().replace(' ', '_'), inplace=True)

cols_ = ['statefp', 'countyfp', 'geoid',
         'name', 'stusps',
         'herb_avgme', 'herb_sdstd', 'pixelscoun', 'vi_doymean', 'vi_doymedi',
         'vi_doystde', 'vi_doymin', 'vi_doymax']
...
# %%
County_NDVIDOY_Herb.rename(columns={"statefp": "state_fips",
...
# %%
County_NDVIDOY_Herb = rc.correct_state_int_fips_to_str(df=County_NDVIDOY_Herb, col_="state_fips")
County_NDVIDOY_Herb = rc.correct_4digitFips(df=County_NDVIDOY_Herb, col_="county_fips")

County_NDVIDOY_Herb = rc.correct_2digit_countyStandAloneFips(df=County_NDVIDOY_Herb, \
                                                             col_="county_fips_standAlone")
County_NDVIDOY_Herb.head(2)

# %%
County_NDVIDOY_Herb.drop(columns=["state_fips", "state",
                                  "county_fips_standAlone", "county_name"],
                        inplace=True)
