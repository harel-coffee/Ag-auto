...
# Field 2017 #
# HN: These are selecting non-irrigated?
#     Also why the 
#                 =="25            and 
#                "ACRES HARVESTED" and 
#               'SPRING', 'WINTER', ... are done twice?

field_25 <- d[d$CENSUS_TABLE == 25,]
field_25 <- field_25[field_25$SHORT_DESC %like% "ACRES HARVESTED",]
field_25 <- field_25 %>% filter(str_detect(SHORT_DESC, "IRRIGAT", negate=TRUE))

field_25 <- field_25 %>% filter(str_detect(SHORT_DESC, "SPRING", negate=TRUE)) %>%
...
field_25 <- field_25 %>% select(CENSUS_CHAPTER, CENSUS_TABLE, STATE_ALPHA, CENSUS_ROW, fips, SHORT_DESC, VALUE)
...
field_merge2$harv <- field_merge2$VALUE_I
...
gl_i <- subset(gl, SHORT_DESC %in% c("HAY & HAYLAGE, IRRIGATED - ACRES HARVESTED","GRASSES & LEGUMES TOTALS, IRRIGATED, SEED - ACRES HARVESTED",

                                     "CORN, SILAGE, IRRIGATED - ACRES HARVESTED", "SORGHUM, SILAGE, IRRIGATED - ACRES HARVESTED"))

gl <- subset(gl, SHORT_DESC %in% c("HAY & HAYLAGE - ACRES HARVESTED","GRASSES & LEGUMES TOTALS, SEED - ACRES HARVESTED",

                                   "CORN, SILAGE - ACRES HARVESTED", "SORGHUM, SILAGE - ACRES HARVESTED"))
