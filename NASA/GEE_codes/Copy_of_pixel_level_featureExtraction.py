...
shapeFile_path = "/content/drive/MyDrive/P.h.D_Projects/" + \
                "Tillage_Mapping/Data/GIS_Data/WSDA_survey/WSDA_checkedForPins.dbf"
...
def cloudMaskL8(image):
    ...
    return image.updateMask(cloud1).updateMask(cloud2)\
                .updateMask(cloud3).updateMask(mask2)\
...
# It will produce a list of imageCollections for each year. Each imageCollection 
# contains the season-based composites for each year.
...
def addDOY(img):
    doy = img.date().getRelative('day', 'year');
     ddoyBand = ee.Image.constant(doy).uint16().rename('doy')
    doyBand
...
def groupImages(year, orgCollection):
  # This groups images and rename bands
    bands = ['B', 'G', 'R', 'NIR', 'SWIR1', \
             'SWIR2', 'evi', 'gcvi', 'ndvi', \
             'sndvi', 'ndti', 'ndi5', 'ndi7', 'crc', 'sti', 'doy'];

    new_bandS0 = ['B_fall', 'G_fall', 'R_fall', 'NIR_fall', \
                  'SWIR1_fall', 'SWIR2_fall', 'evi_fall', \
                  'gcvi_fall', 'ndvi_fall', 'sndvi_fall', \
                  'ndti_fall', 'ndi5_fall', 'ndi7_fall', \
                  'crc_fall', 'sti_fall', 'doy_fall'];
    
    new_bandS1 = ['B_spring', 'G_spring', 'R_spring', \
                  'NIR_spring', 'SWIR1_spring', \
                  'SWIR2_spring', 'evi_spring',\
                  'gcvi_spring', 'ndvi_spring', \
                  'sndvi_spring', 'ndti_spring', \
                  'ndi5_spring', 'ndi7_spring', \
                  'crc_spring', 'sti_spring', 'doy_spring'];
    
    # new_bandS2 = ['B_S2', 'G_S2', 'R_S2', 'NIR_S2', 'SWIR1_S2', \
    # 'SWIR2_S2', 'evi_S2', 'gcvi_S2', 'ndvi_S2', 'sndvi_S2', 'ndti_S2', \
    # 'ndi5_S2', 'ndi7_S2', 'crc_S2', 'sti_S2', 'doy_S2'];
    # new_bandS3 = ['B_S3', 'G_S3', 'R_S3', 'NIR_S3', 'SWIR1_S3', 'SWIR2_S3', \
    #               'evi_S3', 'gcvi_S3', 'ndvi_S3', 'sndvi_S3', 'ndti_S3', \
    #               'ndi5_S3', 'ndi7_S3', 'crc_S3', 'sti_S3', 'doy_S3'];
  
    year = ee.Number(year)
    collection_1 = orgCollection.filterDate(ee.Date.fromYMD(year, 9, 1), 
                                            ee.Date.fromYMD(year, 12, 30)
                                           ).filterBounds(geometry)\
                                .map(addDOY)\
    ...
    # .map(lambda img: img.set('system:time_start', ee.Date.fromYMD
    # (ee.Number(year).add(ee.Number(1)), 1, 1).millis()))

  # Return a list of imageCollections
  return [collection_1, collection_2]

# ///// Function to rename the bands of each composite 
# in the imageCollections associated with each year ///// 
def renameComposites(collectionList):
  ...
  for i in range(len(collectionList)):
    ...
    bands = ['B', 'G', 'R', 'NIR', 'SWIR1', 'SWIR2', 'evi', \
             'gcvi', 'ndvi', 'sndvi', 'ndti', 'ndi5', 'ndi7', \
             'crc', 'sti'];
    new_bandS0 = ['B_fall', 'G_fall', 'R_fall', 'NIR_fall', 
                  'SWIR1_fall', 'SWIR2_fall', 'evi_fall', 'gcvi_fall', 
                  'ndvi_fall', 'sndvi_fall', 'ndti_fall', 'ndi5_fall', 
                  'ndi7_fall', 'crc_fall', 'sti_fall'];
    new_bandS1 = ['B_spring', 'G_spring', 'R_spring', 'NIR_spring', 
                  'SWIR1_spring', 'SWIR2_spring', 'evi_spring', 
                  'gcvi_spring', 'ndvi_spring', 'sndvi_spring', 
                  'ndti_spring', 'ndi5_spring', 'ndi7_spring', 'crc_spring', 'sti_spring']; 
...
def mainBand_pixelExtractor(imgcollection):
  ...
  for idx, img in enumerate(imageList): 
    # idx = idx + 1
    imageList_names = []
    # Used for saving each composite with it's related name "B_Sx" where x = 0, 1 or more"
  ...
  for idx, img in enumerate(imageList[1:]): 
    ...
    for idx, f in enumerate(features_geopandasList):
      ...
      task = ee.batch.Export.table.toDrive(**{
                                 'collection': pixel_featureCollection,
                                 'description': year + "_" + str(int(year) + 1) + \
                                 '_glcmBand_' + first_BandName.split("_")[1] + '_polygon' + f'{idx}',
                                 'folder': 'seasonBased_pixel_level_glcmBands_TillageData',
                                 'fileNamePrefix': year + "_" + str(int(year) + 1) + 
                                 '_glcmBand_' + first_BandName.split("_")[1] + '_polygon' + f'{idx}',
                                        'fileFormat': 'CSV'})
      task.start()
  return imageList_names
...
# ///// Functions to extract pixel values of each image 
# (distribution-based composite) and save them to drive.
# ///// This function loops over each each image and each polygon.
# For main bands:
def metricBased_mainBand_pixelExtractor(imgList, percentile):
  ...
  for idx, img in enumerate(imgList[1:]):
    idx = idx + 1 
    imageList_names = []
    # Used for saving each composite with it's related name, e.g. "B_fall_p5"
    first_BandName = ee.Image(imgList[idx]).bandNames().get(0).getInfo() 
    imageList_names = imageList_names + [first_BandName]
    print(first_BandName)
    for idx, f in enumerate(features_geopandasList):
      #idx = idx + 564
      ...
      task = ee.batch.Export.table.toDrive(**{
                            'collection': pixel_featureCollection,
                            'description': year + "_" + str(int(year) + 1) + 
                            '_mainBand_' + first_BandName.split("_")[1] + "_P" + 
                             str(percentile) + '_polygon' + f'{idx}',
...
def metricBased_glcmBand_pixelExtractor(imgList, percentile):
    ...
    for img_idx, img in enumerate(imgList):
        ...
        imageList_names = []
        # Used for saving each composite with it's related name "B_Sx" where x = 0, 1 or more"
        first_BandName = ee.Image(imgList[img_idx]).bandNames().get(0).getInfo() 
        imageList_names = imageList_names + [first_BandName]
        print(first_BandName)
        for idx, f in enumerate(features_geopandasList):
            # idx = idx + 267
            ...
            task = ee.batch.Export.table.toDrive(**{
                        'collection': pixel_featureCollection,
                        'description': year + "_" + str(int(year) + 1) + '_glcm_' + \
                                      first_BandName.split("_")[1] + "_P" +str(percentile) + \
                                    '_polygon' + f'{idx}',
                        'folder': 'DistributionBased_glcmBands_TillageData',
                        'fileNamePrefix': year + "_" + str(int(year) + 1) + \
                                         '_glcm_' + first_BandName.split("_")[1] + \
                                         "_P" +str(percentile) +  '_polygon' + f'{idx}',
                        'fileFormat': 'CSV'})
            task.start()
    return imageList_names
