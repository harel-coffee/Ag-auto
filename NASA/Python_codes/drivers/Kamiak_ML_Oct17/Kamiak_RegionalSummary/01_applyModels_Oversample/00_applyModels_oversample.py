...
print(
    "Passed Args. are: size={}, VI_idx={}, smooth={}, county_={}, model={}".format(
        size, VI_idx, smooth, county_, model
    )
)
...
model_dir_base = NASA_base + "ML_Models_Oct17/overSample/"
out_dir = dir_base + "02_ML_preds_oversampled/"
...
print("In current county len(SF_data.ID.unique()): {}".format(len(SF_data.ID.unique())))

SF_data = nc.filter_out_nonIrrigated(SF_data)
print("Irrigated Fields len(SF_data.ID.unique()): {}".format(len(SF_data.ID.unique())))
