...
f_names = [x for x in f_names if x.endswith(".csv")]
patt_ = indeks + "_pre2008"
f_names = [x for x in f_names if patt_ in x]

L578 = pd.DataFrame()

for a_file_n in f_names:
    a_file = pd.read_csv(data_dir + a_file_n)
    L578 = pd.concat([L578, a_file])
...
out_name = output_dir + "noOutlier_" + indeks + "_pre2008.csv"
