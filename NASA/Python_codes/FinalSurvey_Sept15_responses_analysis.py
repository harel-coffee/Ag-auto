...
easyVI_disagreements_forFinalSurvey_Sept15.drop(
    labels=["Hossein", "Supriya", "Kirti", "Mike", "Vote"], axis="columns", inplace=True
)

# %%
response_set_xl = pd.ExcelFile(
    param_dir + "NE_final_survey/" + "easy_fiels_finalSurvey_Sept15_responses.xlsx"
)
...
all_responses = pd.DataFrame(columns=response_cols, index=range(question_count))
row_number = -1
        ...
        if "QUESTION" in col:
            ...
            for email in sample_response["Email Address"].values:
                if "consensus" in email:
                    all_responses.loc[row_number, "ConsensusV"] = curr_tbl[
                        curr_tbl["Email Address"] == email
                    ].values[0][1]

                    all_responses.loc[row_number, "Marked"] = curr_tbl[
                        curr_tbl["Email Address"] == email
                    ].values[0][2]

                elif "wsu" in email:
                    all_responses.loc[row_number, "MikeV"] = curr_tbl[
                        curr_tbl["Email Address"] == email
                    ].values[0][1]
...
# %%
all_responses = pd.merge(
    all_responses, easyVI_disagreements_forFinalSurvey_Sept15, on=["ID"], how="left"
)
...
out_name = (
    param_dir + "NE_final_survey/analysis/" + "agreementsOnFinalSurveySept15_RecordedonSept20.csv"
)
...
# %%
VI_TS_dir = (
    "/Users/hn/Documents/01_research_data/NASA/VI_TS/06_SOS_plots/exactEval6000_vertLine_flat/"
)
out_dir_base = (
    "/Users/hn/Documents/01_research_data/NASA/final_survey_potMeetingPlots_for_anotherMeeting/"
)
...
for a_row in MikeDisagreement.index:
    plant = (
        MikeDisagreement.loc[a_row, "CropTyp"]
        .lower()
        .replace(" ", "_")
        .replace(",", "")
        .replace("/", "_")
