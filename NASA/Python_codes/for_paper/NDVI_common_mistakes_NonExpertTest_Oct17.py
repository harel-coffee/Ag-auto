...
# %% [markdown]
# ## Read test set and then figure which ones are labeled by non-experts
...
# %%
SG_preds = pd.merge(SG_preds, GT_labels, on=['ID'],how='left')
regular_preds = pd.merge(regular_preds, GT_labels, on=['ID'],how='left')
...
# %%
regular_preds[~(regular_preds.RF_NDVI_regular_preds == regular_preds.Vote)]
...
for anID in list(reg_common_mistake_fields):
    ...
    final_plot_path = plot_dir + "NDVI_regular_nonExpert/"
    os.makedirs(final_plot_path, exist_ok=True)
    fig_name = final_plot_path + anID + '_reg_NonExpert.pdf'
    plt.savefig(fname = fig_name, dpi=400, bbox_inches='tight')
    plt.close('all')

# %%
SG_common_mistake_fields = SG_preds[~(SG_preds.RF_NDVI_SG_preds == \
                                                 SG_preds.Vote)]
...
for anID in list(SG_common_mistake_fields):
    ...
    curr_plt = plot_oneColumn_CropTitle(dt=curr_smooth, raw_dt=curr_raw, idx="NDVI",
                                        titlee=title_, _label = VI_idx + " (5-step smoothed)")

    final_plot_path = plot_dir + "NDVI_SG_nonExpert/"
    os.makedirs(final_plot_path, exist_ok=True)
    fig_name = final_plot_path + anID + '_SG_NonExpert.pdf'
    plt.savefig(fname = fig_name, dpi=400, bbox_inches='tight')
    plt.close('all')

# %%
for anID in list(SG_common_mistake_fields):
    curr_smooth_reg = regular_TS[regular_TS.ID == anID]
    curr_smooth_SG = SG_TS[SG_TS.ID == anID]

    curr_raw = landsat_raw[landsat_raw.ID == anID]
    curr_year = curr_smooth_reg.human_system_start_time.dt.year.unique()[0]
    curr_raw = curr_raw[curr_raw.human_system_start_time.dt.year==curr_year]
    ...
    fig, ax = plt.subplots(1, 1, figsize=(15, 4), sharex=False, sharey='col', # sharex=True, sharey=True,
                               gridspec_kw={'hspace': 0.35, 'wspace': .05});
    ax.grid(True);
    ax.plot(curr_smooth_reg['human_system_start_time'], curr_smooth_reg[VI_idx], 
            linewidth=4, color = "dodgerblue", label = VI_idx + " (4-step smoothed)") 

    ax.plot(curr_smooth_SG['human_system_start_time'], curr_smooth_SG[VI_idx], 
            linewidth=4, color = "#2ca02c", linestyle = "dotted", label = VI_idx + " (5-step smoothed)") 


    ax.scatter(curr_raw['human_system_start_time'], curr_raw[VI_idx], s=20, c="r", label="raw")

    titlee = list(curr_meta.CropTyp)[0] + " (" + str(list(curr_meta.Acres)[0]) + " acre), "+ \
                 "Experts' vote: " + str(curr_vote) + ", prediction: " + str(curr_pred)
    ...
    print (fig_name)
    plt.show()
    fig.savefig(fname = fig_name, dpi=400, bbox_inches='tight')
    print (fig_name)
    plt.close('all')
