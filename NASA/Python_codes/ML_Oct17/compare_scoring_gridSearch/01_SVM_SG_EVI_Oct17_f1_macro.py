...
ground_truth_labels = ground_truth_labels_extended[
    ground_truth_labels_extended.ExctAcr >= 10
].copy()
...
ground_truth_wide = pd.DataFrame(columns=columnNames, index=range(len(ground_truth_TS.ID.unique())))
...
SVM_classifier_balanced_00 = GridSearchCV(
    SVC(random_state=0, class_weight="balanced"), parameters, cv=5, verbose=1, scoring="f1_macro"
)
...
SVM_classifier_NoneWeight_00 = GridSearchCV(
    SVC(random_state=0), parameters, cv=5, verbose=1, scoring="f1_macro"
)
...
# %%
SVM_classifier_NoneWeight_00_predictions = SVM_classifier_NoneWeight_00.predict(
    x_test_df.iloc[:, 1:]
)
...
balanced_confus_tbl_test = pd.DataFrame(
...
None_confus_tbl_test = pd.DataFrame(
    columns=["None", "Predict_Single", "Predict_Double"], index=range(2)
)
...
None_y_test_df_act_1_pred_2 = None_y_test_df_act_1_pred_2[
    None_y_test_df_act_1_pred_2.prediction == 2
].copy()
None_y_test_df_act_2_pred_1 = None_y_test_df_act_2_pred_1[
    None_y_test_df_act_2_pred_1.prediction == 1
].copy()

# %%
None_y_test_df_act_2_pred_1 = pd.merge(
    None_y_test_df_act_2_pred_1, ground_truth_labels_extended, on=["ID"], how="left"
)
None_y_test_df_act_1_pred_2 = pd.merge(
    None_y_test_df_act_1_pred_2, ground_truth_labels_extended, on=["ID"], how="left"
)
...
SVM_classifier_gridWeight = GridSearchCV(
    SVC(random_state=0), parameters, cv=5, verbose=1, error_score="raise", scoring="f1_macro"
)
