...
# define grid search
SVM_classifier_NEW = GridSearchCV(estimator=SVC(gamma='scale', random_state=0), 
                                          param_grid=param_grid, 
                                          n_jobs=-1, cv=cv, scoring='roc_auc',
                                          verbose=1)

SVM_classifier_NEW.fit(x_train_df.iloc[:, 1:], y_train_df.Vote.values)

# report the best configuration
print("Best: %f using %s" % (SVM_classifier_NEW.best_score_, 
                             SVM_classifier_NEW.best_params_))

# report all configurations
means = SVM_classifier_NEW.cv_results_['mean_test_score']
stds = SVM_classifier_NEW.cv_results_['std_test_score']
params = SVM_classifier_NEW.cv_results_['params']

print ()
print (SVM_classifier_NEW.best_params_)
print (SVM_classifier_NEW.best_score_)

# %% [markdown]
# ### Test

# %%
SVM_classifier_NEW_preds = SVM_classifier_NEW.predict(x_test_df.iloc[:, 1:])

NEW_y_test_df=y_test_df.copy()
NEW_y_test_df
NEW_y_test_df["prediction"] = list(SVM_classifier_NEW_preds)
confusion_matrix(New_y_test_df.Vote, NEW_y_test_df.prediction)

# %%
print(classification_report(NEW_y_test_df.Vote, SVM_classifier_NEW_preds))
...
# define grid search
SVM_classifier_NEW_ACC = GridSearchCV(estimator=SVC(gamma='scale', random_state=0), 
                                      param_grid=param_grid, 
                                      n_jobs=-1, cv=cv,
                                      verbose=1)

SVM_classifier_NEW_ACC.fit(x_train_df.iloc[:, 1:], y_train_df.Vote.values)

# report the best configuration
print("Best: %f using %s" % (SVM_classifier_NEW_ACC.best_score_, 
                             SVM_classifier_NEW_ACC.best_params_))

# report all configurations
means = SVM_classifier_NEW_ACC.cv_results_['mean_test_score']
stds = SVM_classifier_NEW_ACC.cv_results_['std_test_score']
params = SVM_classifier_NEW_ACC.cv_results_['params']
# for mean, stdev, param in zip(means, stds, params):
#     print("%f (%f) with: %r" % (mean, stdev, param))

# %%
SVM_classifier_NEW_ACC_preds = SVM_classifier_NEW_ACC.predict(x_test_df.iloc[:, 1:])

NEW_y_test_df_ACC=y_test_df.copy()
NEW_y_test_df_ACC["prediction"] = list(SVM_classifier_NEW_ACC_preds)
confusion_matrix(NEW_y_test_df_ACC.Vote, NEW_y_test_df_ACC.prediction)
