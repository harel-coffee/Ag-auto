...
for (a_fng_emission in unique(CH_dt$emission)){
  for (a_fng_loc in unique(CH_dt$location)){
    curr <- CH_dt %>% filter(location==a_fng_loc & emission==a_fng_emission) %>% data.table()
    ...
    ggsave(plot = plt,
           filename=paste0(a_fng_loc, "_", a_fng_emission, "_2025_2049_CH_and_tMin_Heat_", "scale_fill_viridis", ".pdf"),
